...
<div class="main-panel">
    ...
    <?php
        for ($i=1; $i < $user_data['field_no']; $i++) { 
            echo '<div class="healthData"><span id="healthlabel'.$i.'"></span><span style="margin-left: 170px;" id="recentValue'.$i.'"></span><br>
            <script>
   ...
   .then(response => 
   {
      ...
      document.getElementById("recentValue'.$i.'").innerHTML = "Recent Reading : "+JSON.stringify(response["feeds"]).replace(/"/g, "");
      
   });
      </script>
            <iframe width="100%" height="240" style="border: 1px solid #cccccc;" src="https://thingspeak.com/channels/';
            echo $user_data["channel_id"];
            echo '/charts/'.$i.'?api_key=EFISS04IFA6364N9&width=auto&color=00FFFF"></iframe></div>';
        }
    ?>
    </div>
